...
`);
  ...
  it('skips node_modules directory', async () => {
    write(dir, 'node_modules/some-package/infra.tf', `
...
`);
  ...
  it('parses JSON CloudFormation template', async () => {
    write(dir, 'template.json', JSON.stringify({
      AWSTemplateFormatVersion: '2010-09-09',
      Resources: {
        EventsQueue: {
          Type: 'AWS::SQS::Queue',
...
describe('extractCDKSchema', () => {
  ...
  it('reads synthesized templates from cdk.out/', async () => {
    write(dir, 'cdk.out/MyStack.template.json', JSON.stringify({
      AWSTemplateFormatVersion: '2010-09-09',
      Resources: {
        OrdersQueue: {
          Type: 'AWS::SQS::Queue',
  ...
  it('reads multiple synthesized stack templates', async () => {
    write(dir, 'cdk.out/StackA.template.json', JSON.stringify({
      AWSTemplateFormatVersion: '2010-09-09',
      Resources: {
        TableA: { Type: 'AWS::DynamoDB::Table', Properties: { TableName: 'TableA', KeySchema: [{ AttributeName: 'id', KeyType: 'HASH' }] } },
      },
    }));
    write(dir, 'cdk.out/StackB.template.json', JSON.stringify({
      AWSTemplateFormatVersion: '2010-09-09',
      Resources: {
        TableB: { Type: 'AWS::DynamoDB::Table', Properties: { TableName: 'TableB', KeySchema: [{ AttributeName: 'id', KeyType: 'HASH' }] } },
      },
    }));
    const schema = await extractCDKSchema(dir);
