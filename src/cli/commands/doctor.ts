...
export async function runDoctor(options: { config?: string } = {}): Promise<void> {
  ...
  // Config file
  results.push(
    await runCheck('Checking config file...', async () => {
  ...
  results.push(
    await runCheck('Validating config...', async () => {
  ...
  // AWS credentials
  results.push(
    await runCheck('Checking AWS credentials...', async () => {
  ...
  // DynamoDB
  results.push(
    await runCheck('Testing DynamoDB access...', async () => {
  ...
  // SQS
  results.push(
    await runCheck('Testing SQS access...', async () => {
  ...
  // SNS
  results.push(
    await runCheck('Testing SNS access...', async () => {
  ...
  // SSM
  results.push(
    await runCheck('Testing SSM Parameter Store access...', async () => {
  ...
  // Secrets Manager
  results.push(
    await runCheck('Testing Secrets Manager access...', async () => {
  ...
  // Lambda
  results.push(
    await runCheck('Testing Lambda access...', async () => {
  ...
  // EventBridge
  results.push(
    await runCheck('Testing EventBridge access...', async () => {
  ...
  // S3
  results.push(
    await runCheck('Testing S3 access...', async () => {
      ...
      try {
        ...
        return {
          ...
          message: err instanceof Error ? err.message : String(err),
          detail:
            'Check IAM: s3:ListBuckets, s3:GetBucketNotificationConfiguration, s3:GetBucketVersioning, s3:GetBucketEncryption, s3:GetPublicAccessBlock',
        };
      }
    }),
  );
  ...
  // CloudWatch Logs
  results.push(
    await runCheck('Testing CloudWatch Logs access...', async () => {
  ...
  // PostgreSQL
  results.push(
    await runCheck('Testing PostgreSQL...', async () => {
  ...
  // MySQL
  results.push(
    await runCheck('Testing MySQL...', async () => {
  ...
  // MongoDB
  results.push(
    await runCheck('Testing MongoDB...', async () => {
  ...
  // Project type
  results.push(
    await runCheck('Detecting project type...', async () => {
  ...
  // IaC detection
  results.push(
    await runCheck('Detecting IaC files...', async () => {
      const cwd = process.cwd();
      const hasTfFile = (dir: string) =>
        fs.existsSync(dir) && fs.readdirSync(dir).some((f) => f.endsWith('.tf'));
      const hasTF =
        hasTfFile(cwd) ||
      ...
      const hasCFN =
        fs.existsSync(path.join(cwd, 'template.yaml')) ||
      ...
      return {
        ...
        detail:
          found.length === 0 ? 'IaC analysis will be skipped without TF/CFN/CDK files' : undefined,
      };
    }),
  );
  ...
  // Cache
  results.push(
    await runCheck('Checking analysis cache...', async () => {
  ...
  if (failed === 0) {
    console.log(
      `  ${chalk.green.bold('All checks passed')}  ${chalk.dim(`${passed} passed, ${warned} warning(s)`)}`,
    );
  } else {
    console.log(
      `  ${chalk.red.bold(`${failed} check(s) failed`)}  ${chalk.dim(`${passed} passed, ${warned} warning(s)`)}`,
    );
  }
  console.log('');
