import type { SystemGraph, Finding, GraphNode } from '../types.js';
 ...
 */
export async function FullTableScanAnalyzer(graph: SystemGraph): Promise<Finding[]> {
 ...
 */
export async function MissingGSIAnalyzer(graph: SystemGraph): Promise<Finding[]> {
  const findings: Finding[] = [];
 ...
 */
export async function HotPartitionAnalyzer(
  graph: SystemGraph,
  hotThreshold = 5,
  tableThresholds: Record<string, number> = {},
): Promise<Finding[]> {
  const findings: Finding[] = [];
  ...
  for (const [tableId, accessors] of tableAccessCount) {
    ...
    const threshold = tableThresholds[tableNode.name] ?? hotThreshold;
