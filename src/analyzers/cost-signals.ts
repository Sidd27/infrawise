...
export async function LambdaHighMemoryAnalyzer(graph: SystemGraph): Promise<Finding[]> {
...
export async function RDSMultiAZNonProdAnalyzer(graph: SystemGraph): Promise<Finding[]> {
  const findings: Finding[] = [];
  for (const node of graph.nodes) {
