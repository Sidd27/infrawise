...
export async function RDSPubliclyAccessibleAnalyzer(graph: SystemGraph): Promise<Finding[]> {
...
export async function RDSNoBackupAnalyzer(graph: SystemGraph): Promise<Finding[]> {
  const findings: Finding[] = [];
  for (const node of graph.nodes) {
    if (node.type !== 'database_instance') continue;
...
export async function RDSUnencryptedAnalyzer(graph: SystemGraph): Promise<Finding[]> {
  const findings: Finding[] = [];
  for (const node of graph.nodes) {
    if (node.type !== 'database_instance') continue;
...
export async function RDSNoDeletionProtectionAnalyzer(graph: SystemGraph): Promise<Finding[]> {
  const findings: Finding[] = [];
  for (const node of graph.nodes) {
    if (node.type !== 'database_instance') continue;
...
export async function RDSNoMultiAZAnalyzer(graph: SystemGraph): Promise<Finding[]> {
  const findings: Finding[] = [];
  for (const node of graph.nodes) {
    if (node.type !== 'database_instance') continue;
    if (!node.multiAZ) {
