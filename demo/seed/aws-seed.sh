...
# LegacyOrders — deployed but NOT in Terraform (triggers IaC drift)
$AWS dynamodb create-table \
...
# temp-processing-queue — deployed but NOT in Terraform (triggers IaC drift)
...
$AWS ssm put-parameter \
  ...
  --value '{"newCheckout": true, "betaDashboard": false}' \
  --type String \
  --overwrite --no-cli-pager 2>/dev/null || true
...
# No rotation (triggers MissingSecretRotationAnalyzer)
...
# Create a minimal zip for Lambda
TMPDIR=$(mktemp -d)
...
# No retention policy (triggers MissingLogRetentionAnalyzer)
$AWS logs create-log-group \
  --log-group-name "/aws/lambda/processOrders" \
  --no-cli-pager 2>/dev/null || true

$AWS logs create-log-group \
  --log-group-name "/aws/lambda/generateReport" \
  --no-cli-pager 2>/dev/null || true

# Very long retention — 400 days (triggers MissingLogRetentionAnalyzer low severity)
$AWS logs create-log-group \
...
echo ""
echo "✅ AWS seed complete. Issues seeded:"
echo "   DynamoDB : Orders (no GSI), LegacyOrders (not in IaC)"
echo "   SQS      : orders-queue (no DLQ, unencrypted), payment-events (no DLQ), temp-processing-queue (not in IaC)"
echo "   Secrets  : demo/db-password + demo/stripe-api-key (no rotation)"
echo "   Lambda   : processOrders (128MB), generateReport (128MB + 300s timeout)"
echo "   Logs     : /aws/lambda/processOrders + generateReport (no retention), /app/audit-logs (400 days)"
