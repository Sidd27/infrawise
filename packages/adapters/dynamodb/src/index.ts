...
import type { DynamoTableMetadata, InfrawiseConfig } from '@infrawise/shared';
import { DynamoDBError, logger } from '@infrawise/core';
...
export async function extractDynamoMetadata(
  ...
  try {
    ...
    logger.info(`Found ${tableNames.length} DynamoDB table(s)`);
  } catch (err) {
