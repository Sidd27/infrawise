import { Pool } from 'pg';
import type { PostgresTableMetadata } from '@infrawise/shared';
import { PostgresConnectionError, logger } from '@infrawise/core';
...
export async function extractPostgresMetadata(
  ...
  try {
    ...
    try {
      ...
      logger.info(`Found ${tablesResult.rows.length} PostgreSQL table(s)`);
