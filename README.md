...
Every response carries a `dataHealth` block with a fixed shape: when the infrastructure was read and how long ago, the status of each source behind that answer, whether `cdk.out` has been synthed since, and the command that refreshes. Every key is always present, so nothing has to be inferred from a field's absence — an empty result you can't distinguish from a failed one reads as "no queues need a DLQ" when the truth is "SQS was never listed". A failed source whose `error` carries a streak (`(2 consecutive failures)`) has failed several analyses in a row — a recurring problem, not a one-off blip.
